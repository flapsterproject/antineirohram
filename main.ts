...
// Словарь ключевых слов и ответов для всех пользователей
const RESPONSES = [
...
// Длинные саркастические сообщения про @neirohambot
const BOT_REPLIES = [
...
// Генерация саркастического ответа, если ключевых слов нет
...
serve(async (req: Request) => {
  ...
  if (!msg.from?.is_bot) {
    ...
    if (username === CREATOR_USERNAME) {
      // Анализируем твой текст точно так же, как для обычных пользователей
      const analyzed = analyzeMessage(text);
      replyText = `${analyzed} — мой создатель 😎`;
    } else {
      // Обычные пользователи получают сарказм
